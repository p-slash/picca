    ...
    def process_file(i):
        ...
        for d in dels:
            ...
            if args.res_estimate == 'Gaussian':
                m_z_arr,ll_arr,de_arr,diff_arr,iv_arr, dll_res_arr = split_forest(nb_part,d.dll if not args.linear_lambda else d.dlambda,d.ll,d.de,d.diff,d.iv,first_pixel,dll_reso=d.dll_resmat,linear_binning=args.linear_binning)
            elif args.res_estimate == 'matrix':
                m_z_arr,ll_arr,de_arr,diff_arr,iv_arr, reso_mat_arr, dll_res_arr = split_forest(nb_part,d.dll if not args.linear_lambda else d.dlambda,d.ll,d.de,d.diff,d.iv,first_pixel,reso_matrix=d.reso_matrix,dll_reso=d.dll_resmat,linear_binning=args.linear_binning)
            for f in range(nb_part):
                ...
                if args.res_estimate == 'Gaussian':
                    ll_new,delta_new,diff_new,iv_new,nb_masked_pixel = fill_masked_pixels(d.dll if not args.linear_lambda else d.dlambda,ll_arr[f] if not args.linear_lambda else 10**ll_arr[f],de_arr[f],diff_arr[f],iv_arr[f],args.no_apply_filling)
                elif args.res_estimate == 'matrix':
