...
#    SNR = []
#    thid_to_print = [35191255688284750,35191259391855506,35191263074454040,35191273975448344,35191281181262835]
#    SNR_selected = []
    for p in data.keys():
        l = []
        for d in data[p]:
#            if(d.thid in thid_to_print):
#                print("mean SNR of thid {} : {}".format(d.thid,d.mean_SNR))
#            if(str(d.plate)[0:5] == "70003"):
#                SNR.append(d.mean_SNR)
...
#            if(args.use_constant_weight and (d.fl.mean()<=0.0 or d.mean_SNR<=1.0 )):
            if(args.use_constant_weight and (d.fl.mean()<=0.0 or d.mean_SNR<=minSNR )):

                log.write("INFO: Rejected {} due to negative mean or too low SNR found\n".format(d.thid))
                continue
#            SNR_selected.append(d.mean_SNR)
            l.append(d)
            log.write("{} {}-{}-{} accepted\n".format(d.thid,
                d.plate,d.mjd,d.fid))
        data[p][:] = l
...
#    mask = SNR == None
#    SNR = SNR[~mask]
#    print("mean SNR 70003 {}".format(sp.mean(SNR)))
#    import matplotlib.pyplot as plt
#    plt.hist(SNR_selected,100)
#    plt.savefig("histogram_SNR_selected.pdf",format="pdf")
